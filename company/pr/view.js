(function () {
    ...
    app.controller("defaultCtrl", function ($rootScope, $scope, $http, popup, $location, paramService, $sce) {
        ...
        $scope.getView = function (params) {
            ...
            $http.get("/" + lang + "/api/board.ashx?t=news_view", { params: $scope.params }).success(function (r) {
        ...
        $scope.pathToGo = function (dest, $event) {
            ...
            if (dest == 'list') {
                href("/company/pr/?" + $.param($scope.params));
            } else if (dest == 'prev') {
                href("/company/pr/view/" + $scope.prevnext.prev_b_id + "?" + $.param($scope.params));
            } else if (dest == 'next') {
                href("/company/pr/view/" + $scope.prevnext.next_b_id + "?" + $.param($scope.params));
            }
        };
