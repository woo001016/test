(function () {
	...
	app.controller("defaultCtrl", function ($rootScope , $scope, $http, popup, $location, paramService) {
	    ...
	    $scope.getList = function (params) {
	        ...
	        $http.get("/" + lang + "/api/board.ashx?t=notice", { params: $rootScope.params }).success(function (r) {
	            $scope.list = r.data;
				...
				$scope.totalPage = Math.ceil($scope.total / $scope.params.rowsPerPage);
	    ...
	    $scope.goView = function ($event) {
            ...
            $http.get("/" + lang + "/api/board.ashx?t=hits&id=" + id).success(function () {
                location.href = "/" + lang + "/customer/notice/view/" + id + "?" + $.param($rootScope.params)
	        });
